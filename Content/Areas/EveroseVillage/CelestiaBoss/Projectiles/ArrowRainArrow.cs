...
public class ArrowRainBow : ModProjectile
{
    ...
    public override void AI()
    {
            ...
            if (AttackTimer > 60)
            {
                if (this.OwnedByLocalClient() && AttackTimer == 70)
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Projectile.rotation.ToRotationVector2(),
                        ModContent.ProjectileType<ArrowRainArrow>(), Projectile.damage, Projectile.knockBack, Projectile.owner, ai1: Target.whoAmI);
                }
        ...
        float aimingRotation = -Vector2.UnitY.ToRotation();
...
public class ArrowRainArrow : ModProjectile
{
...
public class MiniCelestialArrowRainer : ModProjectile
{
    private ref float Timer => ref Projectile.ai[0];
    private Player Target => Main.player[(int)Projectile.ai[1]];
    public override void SetDefaults()
    ...
    public override void AI()
    {
        ...
        if (Timer % 7 == 0 && this.OwnedByLocalClient())
        {
            ...
            pos.Y -= 768;
            pos.X += Main.rand.NextFloat(-200, 200);
            pos.X -= MathF.Sign(Projectile.velocity.X) * 512;
            Vector2 vel = new Vector2();
            vel.X = MathF.Sign(Projectile.velocity.X) * 2;
            vel.Y = 3;
            Projectile.NewProjectile(Projectile.GetSource_FromThis(), pos, vel,
                ModContent.ProjectileType<CelestialArrow>(), Projectile.damage, Projectile.knockBack, Projectile.owner, ai2: 1);
        }
    }
    public override bool PreDraw(ref Color lightColor)
    {
