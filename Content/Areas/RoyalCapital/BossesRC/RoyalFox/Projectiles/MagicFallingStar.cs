...
public class MagicFallingStar : ModProjectile,
    IDrawToRenderTarget
{
    ...
    private ref float WillReTarget => ref Projectile.ai[2];
    ...
    private void SwitchState(float state)
    {
        Timer = 0;
        State = state;
    ...
    private void AI_Falling()
    {
        if(Timer == 1)
        {
            ...
            {
                WillReTarget = Main.rand.NextBool(3) ? 1 : 0;
    ...
    private void AI_ReTarget()
    {
        ...
        Player closestPlayer = PlayerHelper.FindClosestPlayer(Projectile.Center, 2048);
        if(closestPlayer != null)
        {
            _targetPosition = closestPlayer.Center;
            Vector2 targetVelocity = (closestPlayer.Center - Projectile.Center);
            targetVelocity = targetVelocity.SafeNormalize(Vector2.Zero);
            targetVelocity *= 8;
            Projectile.velocity = Projectile.velocity.MoveTowards(targetVelocity, MathHelper.Lerp(0f, 3, EasingFunction.InExpo(Timer / 60f)));
        }
    ...
    private void AI_Zoom()
    {
        if(Timer == 1)
        {
            SoundStyle activateSound = AssetRegistry.Sounds.AlcaricFox.FenixStarsactivate;
    ...
    public override bool PreDraw(ref Color lightColor)
    {
       ...
       // Main.spriteBatch.Draw(drawer);
        ...
        float alpha = MathHelper.Lerp(0f, 1f, _flashTimer / 30f);
    ...
    {
        return MathHelper.SmoothStep(20, 13, EasingFunction.InOutSine(completionRatio));
    }
