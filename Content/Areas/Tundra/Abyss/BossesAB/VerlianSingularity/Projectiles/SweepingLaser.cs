...
public class SweepingLaser : VSProjectile
{
    ...
    public override void AI()
    {
        ...
        if (_inTimer == 1)
        {
            SoundStyle laserSound = AssetReferences.Assets.Sounds.SingularityFragment_LAZERFast.Asset;
    ...
    private float WidthFunction(float interpolant)
    {
        ...
        return width * inScale * outScale * Main.rand.NextFloat(0.95f, 1f) * groScale * 0.45f * EasingFunction.OutExpo(interpolant);
    }
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        shader.PrimaryTexture = TrailRegistry.FlamingTrail;
        ...
        pass.Apply();
        TrailDrawer.Draw( _laserPoints, ColorFunction, WidthFunction, pass.Shader);

        shader2.LaserColor = Color.White;
        shader2.OuterColor = Color.Blue;
        shader2.LaserTexture = AssetReferences.Assets.LaserTextures.Aura.Asset;
        shader2.BloomTexture = AssetReferences.Assets.LaserTextures.Aura.Asset;
        TrailDrawer.Draw(Main.spriteBatch, _laserPoints, ColorFunction, WidthFunction, shader2);
        ...
        PixelationManager.QueueSpritebatchDrawAction(DrawFlash, DrawLayer.OverWater);
        return false;
    }
    private void DrawFlash(SpriteBatch spriteBatch, Vector2 sp)
    {
        Vector2 drawScale = Vector2.One;

        Vector2 drawPosition = _laserPoints[_laserPoints.Length - 1] - Main.screenPosition;
        var _sparkTexture = AssetReferences.Assets.GlowMasks.JumbledGlowCircle.Asset;
        Vector2 lightDrawOrigin = _sparkTexture.Size() / 2f;
        float inScale = EasingFunction.InOutSine(_inTimer / 30f);
        float sparkyRot = Projectile.rotation + Main.GlobalTimeWrappedHourly * 4;
        ...
        color.A = 0;
        spriteBatch.Draw(_sparkTexture.Value, drawPosition, null, color * 1f, sparkyRot, lightDrawOrigin, drawScale * 3 * scaleOsc2, SpriteEffects.None, 0);
        spriteBatch.Draw(_sparkTexture.Value, drawPosition, null, color * 0.25f, sparkyRot + 0.2f, lightDrawOrigin, drawScale * 8 * scaleOsc2, SpriteEffects.None, 0);
   

        SpritebatchDrawer muzzleDrawer = SpritebatchDrawer.FromTextureAsset(AssetReferences.Assets.GlowMasks.SimpleGlowCircle.Asset, _laserPoints[0]);
        muzzleDrawer.color = Color.White * EasingFunction.InOutSine(Projectile.timeLeft / 45f);
        muzzleDrawer.color.A = 0;
        spriteBatch.Draw(muzzleDrawer);
    }
}
