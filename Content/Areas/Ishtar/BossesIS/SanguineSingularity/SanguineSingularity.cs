...

{
    ...
    private ref float TakenDamage => ref NPC.ai[3];
    ...
    public override void AI()
    {
        ...
        NoiseTimer++;
        if(MultiplayerHelper.IsHost && Main.rand.NextBool(180))
        {
            _direction = Main.rand.NextVector2Circular(1, 1);
        }
        NPC.velocity = Vector2.Lerp(NPC.velocity, _direction, 0.02f);
        CameraTargetSystem.AddTarget(Vector2.Lerp(Main.LocalPlayer.Center, NPC.Center, 0.25f));
        if(TakenDamage >= MaxDamage || NoiseTimer >= 600)
        {
            //I think setting active false won't kill the other npc?
            //I hope not
            var fx = FXUtil.GlowCircleBoom(NPC.Center, Color.White, Color.Red, Color.DarkRed, duration: 24, baseSize: 0.18f);
            fx.Scale *= 1.8f;
            for (int i = 0; i < 16; i++)
            {
                Particles.SwirlingFlameDust.Spawn(BitDustFactory.SlowingOverTime with
                {
                    ...
                    innerColor = Color.Red.ToVector4(),
        ...
        if (MultiplayerHelper.IsHost && NoiseTimer % 70 == 0)
        {
            Vector2 velocity = Main.rand.NextVector2CircularEdge(1800, 1800);
            Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, velocity,
                ModContent.ProjectileType<BloodGeyser>(), 34, 1, Main.myPlayer, ai1: NPC.whoAmI);
        }
    ...
    private void DrawSingularity(SpriteBatch spriteBatch, Vector2 screenPos, Color color)
    {
        ...
        drawer.color = Color.DarkRed * 0.6f;
        drawer.color.A = 0;
    ...
    private void DrawIncresionDiskBottom(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        Texture2D supernovaTopTexture = ModContent.Request<Texture2D>(
            ModContent.GetInstance<SanguineSingularity>().Texture + "_Disk").Value;
        ...
        float drawScale = NPC.scale * 1.75f;
        drawScale *= 0.4f;
    ...
    private void DrawIncresionDiskTop(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        Texture2D supernovaTopTexture = ModContent.Request<Texture2D>(
            ModContent.GetInstance<SanguineSingularity>().Texture + "_Top").Value;

        //Incresion Disk Draw Color
        Color incresionDiskDrawColor = Color.White;
        incresionDiskDrawColor *= 0.15f;
        incresionDiskDrawColor.A = 0;
        incresionDiskDrawColor *= 1;
    ...
    IDrawOutlines
{
    ...
    public override void ReceiveExtraAI(BinaryReader reader)
    {
        ...
        _takenDamage = reader.ReadSingle();
    }
    ...
    public override bool CanHitPlayer(Player target, ref int cooldownSlot)
    {
        Rectangle smallRect = NPC.getRect();
        smallRect = smallRect.CenterPad(-78);

        return base.CanHitPlayer(target, ref cooldownSlot) && _contactDamage && smallRect.Intersects(target.getRect());
    }
    ...
    private void HandleGores()
    {
        ...
        GoreManager.Update(NPC.Center + GetDrawOffset());
    }
    ...
    private void AI_Tired()
    {
        ...
        if(Timer == 1)
        {
            if (MultiplayerHelper.IsHost)
            {
                NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X, (int)NPC.Center.Y,
                    ModContent.NPCType<HeadlessSingularity>(), ai0: NPC.whoAmI, ai1: NPC.lifeMax / 12);
            }
        ...
        _draw.singularityScale = Vector2.Lerp(Vector2.One, Vector2.Zero, EasingFunction.OutExpo(Timer / 30f));
        if(Timer < 120)
        {
        ...
        {
            _iniitalRushVector = Main.rand.NextVector2Circular(1, 1);
            Vector2 headSpot = HeadSpot();
            float dist = Vector2.Distance(NPC.Center, headSpot);
            if(dist > 384)
            {
                _iniitalRushVector = (headSpot - NPC.Center).SafeNormalize(Vector2.Zero);
            }
            NPC.netUpdate = true;
        }
        _animation = AnimationState.Run;
        NPC.velocity = Vector2.Lerp(NPC.velocity, _iniitalRushVector.SafeNormalize(Vector2.Zero) * 8, 0.1f);
        ...
        if (Timer >= 60 && !MyHeadAlive()) 
        {
            SwitchState(AIState.Idle);
        }
    }

    private Vector2 HeadSpot()
    {
        foreach (var npc in Main.ActiveNPCs)
        {
            if (npc.ai[0] == NPC.whoAmI && npc.type == ModContent.NPCType<HeadlessSingularity>())
                return npc.Center;
        }
        return NPC.Center;
    }
    private bool MyHeadAlive()
    {
        foreach(var npc in Main.ActiveNPCs)
        {
            if (npc.ai[0] == NPC.whoAmI && npc.type == ModContent.NPCType<HeadlessSingularity>())
    ...
    private void CreateRoar()
    {
        Particles.BloodyMurderDust.Spawn(RoarDustData.Default with { position = NPC.Center, timeLeft = 24 });
        for(int i = 0; i < 4; i++)
        {
            Particles.SwirlingFlameDust.Spawn(BitDustFactory.SlowingOverTime with {
                position = NPC.Center,
    ...
    {
        base.HitEffect(hit);
        _takenDamage += hit.Damage;
        if(_takenDamage >= NPC.lifeMax / 6)
        {
            _takenDamage = 0;
            SwitchState(AIState.Tired);
        }
    ...
    private void AI_BloodRain_Rain()
    {
        ...
        if (Timer % 20 == 0)
        {
            if (MultiplayerHelper.IsHost)
            {
                ...
                if(AttackNumber % 2 == 0)
                {
                    spawnPoint.X = MyTarget.Center.X + 138;
                }
