...
namespace Stellamod.WorldG
{
    ...
    public class StellaWorld : ModSystem
    {
        ...
        private void WorldGenManor(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            while (!placed && attempts++ < 10000000)
            {
                ...
                for (int da = 0; da < 1; da++)
                {
                    ...
                    Point ishtarLocation = new Point(smx - 500, smy + 100);
        ...
        private int SurfaceHighest(int x)
        {
            int y = (int)GenVars.worldSurfaceLow + 200;
            int highest = y;
        ...
        public static int SurfaceHighestY(int x)
        {
            int y = (int)GenVars.worldSurfaceLow + 200;
            int highest = y;
            while (y > GenVars.worldSurfaceHigh - 200)
            {
                y--;
                if (WorldGen.SolidTile(x, y))
                {
        ...
        private void WorldGenVeilSpot(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            while (!placed && attempts++ < 10000000)
            {
                int iceBiomeCenter = GenVars.snowOriginLeft + GenVars.snowOriginRight;
                iceBiomeCenter /= 2;
                int iceBiomeTop = SurfaceHighest(iceBiomeCenter);

                Point Loc = new Point(iceBiomeCenter, iceBiomeTop);
                NPCs.Town.AlcadSpawnSystem.LiberatTile = Loc;
                Point Loc7 = new Point(iceBiomeCenter, iceBiomeTop);
                WorldGen.TileRunner(Loc7.X, Loc7.Y, 600, 2, ModContent.TileType<Tiles.CatagrassBlock>(), false, 0f, 0f, true, true);

                pointLil = new Point(iceBiomeCenter, iceBiomeTop);
                placed = true;
            }
        }
        ...
        private void WorldGenVU(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            point.Y = SurfaceHighestY(point.X);
            point.Y += offset + 25;
            while (!placed && attempts++ < 10000000)
            {
        ...
        private void WorldGenCathedral(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            {
                // Select a place in the first 6th of the world, avoiding the oceans
                int towerX = GenVars.snowOriginRight + WorldGen.genRand.Next(0, 100);
        ...
        private void WorldGenTS(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            {
                // Select a place in the first 6th of the world, avoiding the oceans
                int towerX = GenVars.snowOriginLeft;
                towerX += WorldGen.genRand.Next(-150, 0);
                int towerY = (int)Main.worldSurface - 200;
                FallToSolidTile(towerX, ref towerY);
