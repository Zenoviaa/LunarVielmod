	...
	public class EmberParticle : Particle
	{
		...
		public override void AI()
		{
			...
			if (ai[0] <= 0)
			{
				...
				velocity += new Vector2(Main.windSpeedCurrent * (Main.windSpeedCurrent * 3f) * MathHelper.Lerp(1f, 0.1f, Math.Abs(velocity.X) / 6f), 0f);
		...
		{
			Texture2D circle = (Texture2D)ModContent.Request<Texture2D>("ParticleLibrary/Textures/Circle");
			Texture2D ember = (Texture2D)ModContent.Request<Texture2D>("ParticleLibrary/Particles/EmberParticle");
			Texture2D glow = (Texture2D)ModContent.Request<Texture2D>("ParticleLibrary/Textures/SoftGlow");
