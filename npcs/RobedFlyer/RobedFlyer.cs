			...
			if (counter >= 240) 
			{
				...
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					...
					for (int i = 0; i < amountOfProjectiles; ++i)
					{
						...
						int p = Projectile.NewProjectile(NPC.GetSource_FromAI(),NPC.Center.X, NPC.Center.Y, direction.X + A, direction.Y + B, ModContent.ProjectileType<Projectiles.RobedProjectile>(), 11, 1, Main.myPlayer, 0, 0);
