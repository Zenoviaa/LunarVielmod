...
{
    public class InsourceSlot : UIElement
    {
        ...
        public Item Item;
        public InsourceSlot(int slot, int context = ItemSlot.Context.InventoryItem, float scale = 1f)
        {
            _context = context;
            _scale = scale;
            _slot = slot;
        ...
        {
            if (item.ModItem is InsourceItem insource)
            {
                return true;
            }
            if (item.IsAir)
