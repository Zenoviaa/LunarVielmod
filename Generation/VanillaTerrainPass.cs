...
namespace Stellamod.Generation
{
    public class VanillaTerrainPass : GenPass
    {
        ...
        protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            num3 *= _random.Next(90, 110) * 0.005;
            double num4 = num3 + Main.maxTilesY * 0.4;
            num4 *= _random.Next(90, 110) * 0.01;
            ...
            for (int i = 0; i < Main.maxTilesX; i++)
            {
                ...
                while (_random.Next(0, 3) == 0)
                {
                    num4 += _random.Next(-2, 3);
                }
            ...
            int num13 = (int)(Main.rockLayer + Main.maxTilesY) / 2 + _random.Next(-100, 20);
            int lavaLine = num13 + _random.Next(50, 80);
            if (WorldGen.remixWorldGen)
        ...
        private static double GenerateWorldSurfaceOffset(TerrainFeatureType featureType)
        {
            ...
            if ((WorldGen.drunkWorldGen || WorldGen.getGoodWorldGen || WorldGen.remixWorldGen) && WorldGen.genRand.Next(2) == 0)
            {
                switch (featureType)
                {
                    case TerrainFeatureType.Plateau:
                        while (_random.Next(0, 6) == 0)
                        {
                            num += _random.Next(-1, 2);
                        }
                        break;
            ...
            else
            {
                ...
                {
                    case TerrainFeatureType.Plateau:
                        while (_random.Next(0, 7) == 0)
                        {
                            num += _random.Next(-1, 2);
                        }
                        break;
                    case TerrainFeatureType.Hill:
                        while (_random.Next(0, 4) == 0)
                        {
                            num -= 1.0;
                        }
                        while (_random.Next(0, 10) == 0)
