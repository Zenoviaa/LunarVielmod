...
namespace Stellamod.Items.Consumables
{
    internal class SingularityBag : ModItem
    {
        ...
        public override void RightClick(Player player)
        {
            ...
            player.QuickSpawnItem(entitySource, ModContent.ItemType<SpacialDistortionFragments>(), Main.rand.Next(40, 65));
            if (Main.rand.NextBool(2))
            {
                player.QuickSpawnItem(entitySource, ModContent.ItemType<TomeOfTheSingularity>());
            }
            if (Main.rand.NextBool(2))
            {
                player.QuickSpawnItem(entitySource, ModContent.ItemType<VoidBlaster>());
            }
            if (Main.rand.NextBool(2))
            {
                player.QuickSpawnItem(entitySource, ModContent.ItemType<VoidStaff>());
            }
            if (Main.rand.NextBool(2))
            {
                player.QuickSpawnItem(entitySource, ModContent.ItemType<EventHorizon>());
            }
