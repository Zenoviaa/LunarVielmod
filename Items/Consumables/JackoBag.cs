...
namespace Stellamod.Items.Consumables
{
    internal class JackoBag : ModItem
    {
        ...
        public override void RightClick(Player player)
        {
            ...
            player.QuickSpawnItem(entitySource, ModContent.ItemType<WanderingFlame>(), Main.rand.Next(20, 50));
            if (Main.rand.NextBool(2))
            {
                player.QuickSpawnItem(entitySource, ModContent.ItemType<JackoShot>());
            }
            if (Main.rand.NextBool(2))
            {
                player.QuickSpawnItem(entitySource, ModContent.ItemType<StaffOFlame>());
            }
            if (Main.rand.NextBool(2))
            {
                player.QuickSpawnItem(entitySource, ModContent.ItemType<ScarecrowSaber>());
            }
        }
