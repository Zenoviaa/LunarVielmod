...
namespace Stellamod.Items.Weapons.Summon.Orbs
{
    internal class ThePollinator : ModItem
    {
        ...
        public override void UpdateInventory(Player player)
        {
            ...
            if(player.HeldItem.type == ModContent.ItemType<ThePollinator>()
                && player.ownedProjectileCounts[ModContent.ProjectileType<ThePollinatorProj>()]==0)
            {
                var projectile = Projectile.NewProjectileDirect(player.GetSource_FromThis(), player.Center, Vector2.Zero, 
                    ModContent.ProjectileType<ThePollinatorProj>(), Item.damage, Item.knockBack, player.whoAmI);
