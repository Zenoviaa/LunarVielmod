...
namespace Stellamod.NPCs.Bosses.DreadMire
{
    public class DreadFire : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 14; i++)
            {
                Dust.NewDustPerfect(base.Projectile.Center, ModContent.DustType<RuneDust>(), (Vector2.One * Main.rand.Next(1, 12)).RotatedByRandom(19.0), 0, default(Color), 1f).noGravity = true;
            }
            for (int i = 0; i < 40; i++)
            {
        ...
        public override void AI()
        {
            ...
            {
                int dust = Dust.NewDust(Projectile.position + Projectile.velocity, Projectile.width, Projectile.height, ModContent.DustType<RuneDust>(), Projectile.velocity.X * 0.5f, Projectile.velocity.Y * 0.5f);
            ...
            if (Main.rand.NextBool(29))
            {
                int dust3 = Dust.NewDust(Projectile.position + Projectile.velocity, Projectile.width, Projectile.height, ModContent.DustType<RuneDust>(), Projectile.velocity.X * 0.5f, Projectile.velocity.Y * 0.5f);
