...
namespace Stellamod.Core.XixianFlaskSystem
{
    ...
    public class XixianFlask : ModItem
    {
        ...
        {
            var flaskPlayer = Main.LocalPlayer.GetModPlayer<FlaskPlayer>();
            if (!flaskPlayer.HasUnlockedFlask())
            {
                // Here we add a tooltipline that will later be removed, showcasing how to remove tooltips from an item
                var line = new TooltipLine(Mod, "xixianlocked", LangText.Common("XixianFlaskLock"))
                {
        ...
        public override void UpdateInventory(Player player)
        {
            ...
            player.GetModPlayer<FlaskPlayer>().UnlockFlask();
