...
float4 PixelShaderFunction(float2 coords : TEXCOORD0, float4 sampleColor : COLOR0) : COLOR0
{
  ...
  //  fireColor = lerp(fireColor, float4(0.0, 0.0, 0.0, 1.0), 1.0 - coords.y);
   // fireColor += smokeColor * 0.3;
    ...
    fireColor += 0.07;
    fireColor *= 1.5;
    float darken = sin(coords.x * 3.14);
    //fireColor *= 1.5 + sin(smokeColor * 0.8 + coords.y * 16.0+ time * 14.0 );
    //fireColor *= sin(mixed + coords.x * 3.4 - time * 111164.0);
    fireColor *= lerp(0.3, 1.0, coords.y);
    return fireColor * sampleColor  * darken;
