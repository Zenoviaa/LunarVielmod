...
float4 PixelShaderFunction(float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float2 maskCoords = (coords * uImageSize0 - uSourceRect.xy) / maskingTextureSize;
    maskCoords += float2(time * -0.05, time * -0.025);
    if (sampleColor.a > 0)
    {
        float4 maskColor = tex2D(maskTex, maskCoords);
        maskColor.rgb *= maskingColor;
        return maskColor;
    }
    else
    {
