...
namespace Stellamod.Projectiles.Gun
{
    public class SharkAcid : ModProjectile
	{
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 20; i++)
            {
                int num1 = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.GreenFairy, 0f, -2f, 0, default(Color), .8f);
            ...
            for (int i = 0; i < 180; i++)
            {
                int num = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.GreenFairy, 0f, -2f, 0, default(Color), 1.5f);
                Main.dust[num].noGravity = true;
            ...
            for (int i = 0; i < 80; i++)
            {
                int num = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.GreenFairy, 0f, -2f, 0, default(Color), 2.5f);
                Main.dust[num].noGravity = true;
                Main.dust[num].position.X += Main.rand.Next(-50, 51) * .05f - 1.5f;
                Main.dust[num].position.X += Main.rand.Next(-50, 51) * .05f - 1.5f;
                Main.dust[num].velocity = Projectile.DirectionTo(Main.dust[num].position) * 10f;
